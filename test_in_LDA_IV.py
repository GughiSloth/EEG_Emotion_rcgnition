...
for train_index, val_index in kf.split(X_trainval_filtered, y_trainval):
    ...
    lda = LDA(n_components=3, solver='eigen', shrinkage=0.005)
    ...
    callbacks = [
        ...
        ModelCheckpoint(f"fold_{fold}_model_IV.h5", monitor='val_accuracy', save_best_only=True, mode='max', verbose=0)
    ]
    ...
    model.save(f"fold_{fold}_model_IV.h5")

    if val_acc > best_val_acc:
        best_val_acc = val_acc
        best_model_path = f"fold_{fold}_model_IV.h5"
        best_lda = lda
        best_svd = svd
...
models = []
for i in range(1, 6):
    model_path = f"fold_{i}_model_IV.h5"
